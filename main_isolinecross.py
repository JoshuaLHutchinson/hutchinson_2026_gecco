...
@hydra.main(version_base=None, config_path="configs/", config_name="me_isolinecross")
def main(config: DictConfig) -> None:
    ...
    def evaluate_repertoire(random_key, repertoire):
        ...
        dem = (
            jnp.sum((1.0 - repertoire_empty) * error) / jnp.sum(1.0 - repertoire_empty)
        ).astype(float)
    ...
    # Define emitter --> Change based on operator
    variation_fn = functools.partial(
        isolinecross_variation,
        iso_sigma=config.operator.iso_sigma,
        line_sigma=config.operator.line_sigma,
        cross_lambda=config.operator.cross_lambda,
        cross_prob=config.operator.cross_prob,
    )
    ...
    repertoire, emitter_state, is_offspring_added, improvement, random_key = (
        map_elites.init(init_params, centroids, random_key)
    )
    ...
    for i in range(num_loops):
        ...
        # Metrics
        random_key, qd_score_repertoire, dem_repertoire = evaluate_repertoire(
            random_key, repertoire
        )
        ...
        current_metrics["qd_score_repertoire"] = jnp.repeat(
            qd_score_repertoire, log_period
        )
        current_metrics["dem_repertoire"] = jnp.repeat(dem_repertoire, log_period)
        if i == -1:
            current_metrics["ga_offspring_added"] = get_elites(
                current_metrics["is_offspring_added"] + is_offspring_added
            )
            current_metrics["ga_improvement"] = get_elites(
                current_metrics["improvement"] + improvement
            )
        else:
            current_metrics["ga_offspring_added"] = get_elites(
                current_metrics["is_offspring_added"]
            )
            current_metrics["ga_improvement"] = get_elites(
        ...
        log_metrics["ga_offspring_added"] = jnp.sum(
            current_metrics["ga_offspring_added"]
        )
    ...
    if env.behavior_descriptor_length == 2:
        env_steps = (
            jnp.arange(config.num_iterations)
